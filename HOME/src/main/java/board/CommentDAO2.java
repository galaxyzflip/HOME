...
public class CommentDAO2 {
	...
	private static CommentDAO2 instance = new CommentDAO2();
	
	private CommentDAO2() {}
	
	public static CommentDAO2 getInstance() {
		return instance;
	}
	...
	public void insertComment(CommentDTO cmt) {
		...
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement("insert into b_comment(content_num, commenter, commentt, passwd, reg_date, ip, comment_num, ref, re_step, re_level) "
					+ "values(?,?,?,?,?,?,?,?,?,?)");
			...
			pstmt.setInt(8, cmt.getRef());
			pstmt.setInt(9, cmt.getComment_num());
			pstmt.setInt(10, cmt.getComment_num());
